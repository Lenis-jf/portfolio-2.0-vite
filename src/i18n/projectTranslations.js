...
const projectTranslations = {
  es: {
    ...
    "edgeml-football": {
      title: "EdgeML Meets American Football — Pipeline de Análisis Táctico con IA",
      content: [
        null,
        "Hasta el momento este ha sido mi proyecto universitario más ambicioso. Junto a dos compañeros más construimos un pipeline de análisis táctico de fútbol americano pensado para correr en tiempo real sobre dispositivos edge con hardware limitado como el Nvidia Jetson Orin Nano de 8GB.",
        "Ver repositorio",
        "Durante el desarrollo asumí el rol de Scrum Master y tomé gran parte de la responsabilidad técnica definiendo la arquitectura y los patrones de diseño. Estuve siempre a disposición del equipo por si chocaban contra un límite técnico en sus tareas del sprint backlog. Al final terminé haciéndome cargo de partes esenciales debido a dificultades en la colaboración con una parte del equipo original. Desarrollé el flujo para reconocer formaciones ofensivas, la lectura asíncrona de los dorsales, un flujo de video que aprovechara el hardware edge al máximo y, en conjunto con mis otros dos compañeros, entrenamos un modelo YOLO para detectar jugadores y un modelo YOLO Pose para detectar puntos clave del campo y poder hacer transformaciones homográficas más adelante.",
        "Uno de los desafíos más grandes fue encontrar una solución práctica para integrar EasyOCR lidiando con la mala calidad de los clips y el constante ID Switching, por lo cual implementé recortes dinámicos orientados precisamente a la reducción del ruido en los torsos de los jugadores. Además para maximizar la eficiencia decidí entrenar un modelo de clasificación de imágenes capaz de categorizar los recortes entre legibles e ilegibles antes de pasarlos por el OCR, ahorrando así muchísimo costo computacional. Toda esta integración se hizo de forma asíncrona para no bloquear el hilo principal y se potenció enormemente gracias a estrategias de caché con diccionarios seguros en entornos multihilo. Estas estrategias no solo sirvieron para lidiar con el problema del ID Switching y evitar procesar jugadores que técnicamente ya habíamos analizado, sino que también permitieron hacer un reanálisis exclusivo de los dorsales con un nivel de confianza menor al 50 por ciento sin riesgo de romper el flujo.",
        null,
        "El reconocimiento de formaciones fue otro gran hito. Los modelos iniciales sobre imágenes del presnap fallaban por oclusión y perspectiva. Tras crear un motor de homografía para mapear los píxeles a un campo 2D con una vista de pájaro decidí cambiar la lógica a XGBoost. Tomé al jugador Center como ancla espacial para los demás e inyecté valores NaN sintéticos en el entrenamiento para simular jugadores ausentes. Esto nos permitió lidiar con el hecho de que no siempre se reconocen los 11 jugadores impulsando la precisión del modelo impresionantemente.",
        null,
        "Finalmente y muy cerca de la fecha de entrega se me ocurrió aprovechar el hardware del Jetson Orin al máximo para entregar verdadero valor a los entrenadores. Como ellos no tienen conocimientos informáticos diseñé un Web Hub alojado directamente en el dispositivo. El Jetson crea su propio hotspot para que se conecten desde sus teléfonos y accedan a la aplicación web, logrando así mantener todo el servicio completamente independiente del internet, de la nube y de servicios externos. Monté el servidor con Uvicorn y FastAPI y desarrollé el frontend en React JS para un mejor manejo de estados y telemetría en tiempo real.",
        null,
        "Descargar Documentación",
        "Este proyecto me involucró de lleno en la arquitectura de software y el desarrollo de producto con metodologías ágiles. Me sacó de mi zona de confort y me obligó a pensar más allá para el bien del producto final. Ver lo que se puede alcanzar trabajando en equipo y utilizando tecnologías frontera como el Machine Learning y la Visión por Computadora despertó en mí una nueva pasión por desarrollar soluciones tecnológicas en sectores donde nadie lo imagina."
      ]
    }
  },
  de: {
    ...
    "edgeml-football": {
      ...
      content: [
        null,
        "Dies war mein bisher ambitioniertestes Universitätsprojekt. Zusammen mit zwei Kommilitonen haben wir eine Pipeline zur taktischen Analyse von American Football gebaut, die darauf ausgelegt ist in Echtzeit auf Edge Geräten mit begrenzter Hardware wie dem Nvidia Jetson Orin Nano mit 8 GB zu laufen.",
        "Repository anzeigen",
        "Während der Entwicklung übernahm ich die Rolle des Scrum Masters und trug einen Großteil der technischen Verantwortung indem ich die Architektur und die Entwurfsmuster definierte. Ich stand dem Team immer zur Verfügung falls sie bei ihren Sprint Backlog Aufgaben an technische Grenzen stießen. Am Ende übernahm ich wesentliche Teile aufgrund von Schwierigkeiten in der Zusammenarbeit mit einem Teil des ursprünglichen Teams. Ich entwickelte den Ablauf zur Erkennung von Offensivformationen, das asynchrone Lesen von Trikotnummern, einen Videoablauf der die Edge Hardware maximal ausnutzt und gemeinsam mit meinen beiden Kommilitonen trainierten wir ein YOLO Modell zur Spielererkennung sowie ein YOLO Pose Modell zur Erkennung von Schlüsselpunkten auf dem Feld um später homographische Transformationen durchführen zu können.",
        "Eine der größten Herausforderungen war es eine praktische Lösung für die Integration von EasyOCR zu finden um mit der schlechten Videoqualität und dem ständigen ID Switching umzugehen. Deshalb implementierte ich dynamische Zuschnitte die genau auf die Rauschunterdrückung an den Torsi der Spieler abzielten. Um die Effizienz zu maximieren trainierte ich zudem ein Bildklassifikationsmodell das in der Lage ist die Zuschnitte in lesbar und unlesbar zu kategorisieren bevor sie an das OCR weitergegeben werden was enorm viel Rechenleistung sparte. Diese gesamte Integration erfolgte asynchron um den Haupt-Thread nicht zu blockieren und wurde durch Caching Strategien mit threadsicheren Dictionaries massiv verbessert. Diese Strategien dienten nicht nur dazu das Problem des ID Switching zu lösen und die Verarbeitung von bereits analysierten Spielern zu vermeiden sondern ermöglichten auch eine exklusive Neuanalyse von Trikotnummern mit einer Konfidenz von unter 50 Prozent ohne den Ablauf zu gefährden.",
        null,
        "Die Formationserkennung war ein weiterer großer Meilenstein. Anfängliche Modelle auf Bildern vor dem Snap scheiterten an Verdeckungen und der Perspektive. Nachdem ich eine Homographie Engine entwickelt hatte um die Pixel aus der Vogelperspektive auf ein 2D Feld abzubilden entschied ich mich die Logik auf XGBoost umzustellen. Ich nahm den Center Spieler als räumlichen Anker für die anderen und injizierte beim Training synthetische NaN Werte um fehlende Spieler zu simulieren. Dadurch konnten wir gut damit umgehen dass nicht immer alle 11 Spieler erkannt werden was die Genauigkeit des Modells beeindruckend steigerte.",
        null,
        "Kurz vor dem Abgabetermin kam mir schließlich die Idee die Hardware des Jetson Orin maximal auszunutzen um den Trainern einen echten Mehrwert zu bieten. Da sie keine IT Kenntnisse haben entwarf ich einen Web Hub der direkt auf dem Gerät gehostet wird. Der Jetson erstellt einen eigenen Hotspot damit sie sich von ihren Handys aus verbinden und auf die Webanwendung zugreifen können wodurch der gesamte Dienst völlig unabhängig von Internet Cloud und externen Diensten bleibt. Ich richtete den Server mit Uvicorn und FastAPI ein und entwickelte das Frontend in React JS für ein besseres State Management und Echtzeit Telemetrie.",
        null,
        "Dokumentation herunterladen",
        "Dieses Projekt hat mich voll in die Softwarearchitektur und die agile Produktentwicklung eingebunden. Es hat mich aus meiner Komfortzone geholt und mich gezwungen für das Wohl des Endprodukts weiter zu denken. Zu sehen was man durch Teamarbeit und den Einsatz von Spitzentechnologien wie Machine Learning und Computer Vision erreichen kann hat in mir eine neue Leidenschaft geweckt technologische Lösungen in Bereichen zu entwickeln in denen es niemand erwartet."
      ]
    }
  }
};
